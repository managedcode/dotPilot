...
- Send-time provider or model failures must become visible in the active chat immediately; the shell must not keep showing a running-looking transcript or stale `Sending message...` state after the runtime has already failed.
- The startup loading overlay must cover only the active initial hydration attempt; if startup hydration finishes in failure, the shell still needs to exit the loading overlay instead of looking permanently busy.
- Startup hydration status in the shell must be non-blocking; warm-up may be visible as lightweight background status, but the operator must be able to use the app immediately while that parallel work continues.
...
- App startup may show a dedicated splash/loading state, but the startup orchestration itself must remain in `DotPilot.Core` as a background startup task; the Uno layer should only bind to that Core-owned state and cached snapshot instead of kicking off provider or workspace hydration from page events.
- Do not put provider/CLI initialization logic in `OnLoaded`, view activation, or other presentation lifecycle hooks; if startup readiness changes, the shell reacts to Core state and nothing more.
