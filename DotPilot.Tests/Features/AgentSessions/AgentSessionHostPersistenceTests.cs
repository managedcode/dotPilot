...
public sealed class AgentSessionHostPersistenceTests
{
    [Test]
    ...
    private static AgentSessionHostOptions CreateOptions(string root, string clusterId, string serviceId)
    {
        return new AgentSessionHostOptions
        {
    ...
    private static async ValueTask<IHost> StartHostAsync(AgentSessionHostOptions options)
    {
        var host = Host.CreateDefaultBuilder()
            .UseDotPilotAgentSessions(options)
    ...
    private static string CreateRootPath()
    {
        return Path.Combine(
            Path.GetTempPath(),
            "DotPilot.Tests",
            nameof(AgentSessionHostPersistenceTests),
